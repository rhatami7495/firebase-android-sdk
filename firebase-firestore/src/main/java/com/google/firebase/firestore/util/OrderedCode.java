...
/**
 * Java port of OrderedCode (google3/strings/ordered-code.h, google3/strings/ordered-code.cc).
 *
 * <p>Note that this class works with bytes, while C++ works with strings due to obvious reasons.
 * Using C++ ordered code implementation would be compatible with this. Encoded data from Java can
 * be decoded using the C++ implementation and vice versa. Note that we compare the unsigned bytes
 * in Java to get the same effect as C++. Also this class maintains the encoded store and reads DO
 * modify the internal store.
 *
 * <p>This module provides routines for encoding a sequence of typed entities into a byte array. The
 ...
 *
 * <p>This module is often useful when generating multi-part sstable keys that have to be ordered in
 * a particular fashion.
 *
 * <p><b>This class is NOT thread safe.</b>
 *
 * @author harendra@google.com (Harendra Verma)
 */
// TODO(harendra): move to com.google.common
public class OrderedCode {
  ...
  public static final byte[] INFINITY_ENCODED_DECREASING = {invert(ESCAPE2), invert(INFINITY)};
   ...
   * byte array store.
   *
   * <p>Port of OrderedCode::WriteNumIncreasing(string*, uint64). Note that the specified long is
   * treated like a uint64, e.g. {@code new OrderedCode().writeNumIncreasing(-1L).getEncodedBytes()
   ...
   * byte array store.
   *
   * <p>Port of OrderedCode::WriteNumDecreasing(string*, uint64). Note that the specified long is
   * treated like a uint64, e.g. {@code new OrderedCode().writeNumDecreasing(-1L).getEncodedBytes()
   * &lt; new OrderedCode().writeNumDecreasing(Long.MAX_VALUE).getEncodedBytes()}.
   *
   * @see #readNumIncreasing()
   */
   ...
   *
   * <p>Port of Bits::Log2Floor64(uint64) except that only non-negative longs are accepted (which
   * seemed more expected than treating negative longs as if they were uint64s).
   */
  @VisibleForTesting
  ...
  /**
   * Calculates the encoding length in bytes of the signed number n. Port of
   * SignedEncodingLength(int64) from ordered-code.cc.
   */
  @VisibleForTesting
  ...
  /**
   * Port of OrderedCode::WriteSignedNumIncreasing(string*, int64). Note that the specified long is
   * treated like an int64, i.e. {@code new OrderedCode().writeNumIncreasing(-1L).getEncodedBytes()
   ...
   *
   * <p>Port of OrderedCode::ReadSignedNumIncreasing(StringPiece*, int64*).
   *
  ...
  public byte[] readTrailingBytes() {
    ...
    byte[] store = encodedArrays.get(0);
    encodedArrays.remove(0);
    Preconditions.checkState(encodedArrays.isEmpty());
